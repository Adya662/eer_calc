...
def calculate_eer_detailed(ref_entities: List[Dict], hyp_entities: List[Dict]) -> Dict:
    ...
    try:
        ...
        for key in ['correct', 'substitutions', 'insertions', 'deletions',
                    'total_ref', 'total_hyp', 'eer_percentage',
                    'detailed_substitutions', 'detailed_insertions',
                    'detailed_deletions', 'detailed_correct']:
            eer_data.setdefault(key, 0 if 'detailed' not in key else [])
    except json.JSONDecodeError:
        raise ValueError(f"Failed to parse EER JSON from Gemini: {response_text}")
    return eer_data
...
def process_single_call(call_dir: Path) -> Dict:
    ...
    output_file = output_dir / "eer_detailed_results.json"
...
def main():
    ...
    parser.add_argument("--output", default="eer_detailed_comprehensive_results.json", 
                       help="Output JSON file for detailed results")
    parser.add_argument("--csv-output", default="eer_detailed_metrics_summary.csv",
    ...
    try:
        ...
        print(f"⚠️ Error during global entity extraction: {e}")
        print("First 1000 characters of reference text:\n", global_ref_text[:1000])
        print("First 1000 characters of hypothesis text:\n", global_gt_text[:1000])
        print("⚠️ Skipping Global EER calculation due to entity extraction failure.")
        global_ref_entities = []
        global_gt_entities = []
    if not global_ref_entities and not global_gt_entities:
        print("⚠️ Skipping Global EER calculation due to failed entity extraction.")
    else:
        try:
            global_all_eer = calculate_eer_detailed(global_ref_entities, global_gt_entities)
            # Guard: check for malformed EER result
            if not isinstance(global_all_eer, dict) or 'eer_percentage' not in global_all_eer:
                print("⚠️ Skipping Global EER calculation due to malformed EER data:", global_all_eer)
            else:
            ...
            print(f"⚠️ Error during Global EER computation: {e}")
            print("⚠️ Skipping Global EER calculation due to EER computation failure.")
