...
try:
    from metaphone import doublemetaphone  # optional
except Exception:
...
def _tokens_equal(a: str, b: str) -> bool:
    ...
    if vac == vbc:
        return True
    # Treat phonetic equality as exact equality (cost 0) to avoid inflating EER
    if sounds_alike(vac, vbc):
        return True
    # Fallback to prior phonetic forgiveness if optional deps unavailable
    ...
    for attempt in range(max_retries):
        try:
            response = client.chat.completions.create(
                ...
                messages=[
                    {"role": "system", "content": "You are an expert NER and EER evaluator. Follow the instructions exactly and output only valid JSON."},
                    {"role": "user", "content": LLM_EER_INSTRUCTIONS + "\n\nINPUT:\n" + json.dumps(payload, ensure_ascii=False)}
                ]
            )
...
def calculate_global_eer(all_ref_entities: List[Dict], all_hyp_entities: List[Dict]) -> Dict:
    ...
    try:
        response = client.chat.completions.create(
            ...
            temperature=GPT_TEMPERATURE,
            messages=[
                {"role": "system", "content": "You are an expert EER evaluator. Output only valid JSON."},
...
def save_per_call_comparison_summary(call_dir: Path, call_id: str, ref_entities: List[str], 
    ...
    # Rename per-call summary file
    summary_file = output_dir / "entity_comparison_gpt_llm.json"
...
def process_single_call(call_dir: Path, output_suffix: str = OUTPUT_SUFFIX_DEFAULT) -> Dict:
    ...
    save_per_call_comparison_summary(call_dir, call_dir.name, ref_entity_list, 
                                   hyp_entity_list, all_eer, concerned_eer, output_suffix)

    # Persist LLM entity lists for global EER
    # Rename entities file
    llm_entities_file = output_dir / "entities_llm.json"
    ...
    # Removed writing detailed per-call results file
...
def main():
    ...
    # Rename run-level JSON default
    parser.add_argument("--output", default="global_eer_gpt_llm.json", 
                       help="Output JSON file for detailed results")
    # Rename run-level CSV default
    parser.add_argument("--csv-output", default="global_eer_summary_gpt_llm.csv",
    ...
    for cdir in call_dirs:
        try:
            # Updated entities filename for aggregation
            with open(cdir / "output" / "entities_llm.json", 'r', encoding='utf-8') as f:
                data = json.load(f)
    ...
    # Formula not printed as we do not have raw sequences here
    ...
    print(f"Individual call summaries saved in each call's output/entity_comparison_gpt_llm.json")

if __name__ == "__main__":
    # Mirror stdout/stderr to output_llm.txt in addition to console
    try:
        log_path = Path(__file__).parent / "output_llm.txt"
