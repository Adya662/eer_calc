...
def extract_entities_gpt(text: str, max_retries: int = 3) -> List[Dict]:
    """Extract entities using GPT LLM"""
    if not text.strip():
        return []
    ...
    for attempt in range(max_retries):
        try:
            response = client.chat.completions.create(
                model=GPT_MODEL,
                messages=[
                    {"role": "system", "content": "You are an expert NER annotator. Output only a JSON array of entities."},
...
def calculate_eer_detailed(ref_entities: List[Dict], hyp_entities: List[Dict]) -> Dict:
    ...
    # Build a JSON payload for the LLM prompt
    payload = {
        "reference_entities": ref_entities,
        "hypothesis_entities": hyp_entities
    }
    prompt = (
        "You are an expert evaluator. Given two lists of named entities in JSON format "
        "under 'reference_entities' and 'hypothesis_entities', compute the Entity Error Rate "
        "using substitutions, insertions, deletions, and total reference count, then output "
        "ONLY a JSON object with keys: eer_percentage, substitutions, insertions, deletions, "
        "total_ref, total_hyp.\n\n"
    ...
    response = client.chat.completions.create(
        model=GPT_MODEL,
        messages=[
            {"role": "system", "content": "You are an expert evaluator. Output only the JSON metrics."},
            {"role": "user", "content": prompt}
        ]
    )
    response_text = response.choices[0].message.content.strip()
    ...
    response_text = response_text.strip()
    try:
        eer_data = json.loads(response_text)
    except json.JSONDecodeError:
        raise ValueError(f"Failed to parse EER JSON from GPT: {response_text}")
    # Ensure required keys are present even if missing in LLM response
    eer_data.setdefault('correct', 0)
    eer_data.setdefault('detailed_substitutions', [])
    eer_data.setdefault('detailed_insertions', [])
    eer_data.setdefault('detailed_deletions', [])
    eer_data.setdefault('detailed_correct', [])
    return eer_data
...
def process_single_call(call_dir: Path) -> Dict:
    ...
    ref_entities = extract_entities_gpt(ref_text)
    
    print(f"[{call_dir.name}] Extracting entities from hypothesis text...")
    gt_entities = extract_entities_gpt(gt_text)
    ...
    output_file = output_dir / "eer_detailed_results_llm.json"
...
def print_summary_table(all_results: List[Dict]):
    ...
    # Header
    header = f"{'Call ID':<15} {'All EER%':<10} {'C-EER%':<10} {'All S':<6} {'All I':<6} {'All D':<6} {'C-S':<5} {'C-I':<5} {'C-D':<5} {'Ref':<5} {'Hyp':<5} {'C-Ref':<6} {'C-Hyp':<6}"
    print(header)
    print('─' * 120)
    ...
    for result in all_results:
        ...
        row += f"{con_eer['total_ref']:<6} {con_eer['total_hyp']:<6}"
...
def main():
    ...
    parser.add_argument("--output", default="eer_detailed_comprehensive_results_llm.json", 
                       help="Output JSON file for detailed results")
    parser.add_argument("--csv-output", default="eer_detailed_metrics_summary_llm.csv",
    ...
    global_ref_entities = extract_entities_gpt(global_ref_text)
    global_gt_entities = extract_entities_gpt(global_gt_text)
    global_all_eer = calculate_eer_detailed(global_ref_entities, global_gt_entities)

    # Filter concerned entities globally
    ...
    # Add global EER metrics into the comprehensive results and update JSON summary
    comprehensive_results["aggregate_metrics"].update({
        "global_all_entities_eer": round(global_all_eer["eer_percentage"], 2),
        "global_substitutions": global_all_eer["substitutions"],
        "global_insertions": global_all_eer["insertions"],
        "global_deletions": global_all_eer["deletions"],
        "global_total_ref": global_all_eer["total_ref"],
        "global_total_hyp": global_all_eer["total_hyp"],
        "global_concerned_entities_eer": round(global_concerned_eer["eer_percentage"], 2),
        "global_concerned_substitutions": global_concerned_eer["substitutions"],
        "global_concerned_insertions": global_concerned_eer["insertions"],
        "global_concerned_deletions": global_concerned_eer["deletions"],
        "global_concerned_total_ref": global_concerned_eer["total_ref"],
        "global_concerned_total_hyp": global_concerned_eer["total_hyp"]
    })

    # Overwrite the comprehensive JSON file with the updated metrics
    with open(args.output, "w", encoding="utf-8") as f:
        json.dump(comprehensive_results, f, indent=2)
    ...
    print(f"📈 Average All Entities EER: {comprehensive_results['aggregate_metrics']['average_all_entities_eer']:.2f}%")
