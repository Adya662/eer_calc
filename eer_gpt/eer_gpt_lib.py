...
def calculate_eer_detailed(ref_entities: List[Dict], hyp_entities: List[Dict]) -> Dict:
    ...
    # EER = (S + I + D) / reference count
    total_errors = S + I + D
    eer = (total_errors / N_ref) * 100 if N_ref > 0 else 0
...
def main():
    ...
    # Calculate Global Concerned EER
    ...
    print(f"📁 Detailed results: {args.output} (lib version)")
    print(f"📋 CSV summary: {args.csv_output} (lib version)")
