...
def save_per_call_comparison_summary(call_dir: Path, call_id: str, ref_entities: List[str], 
    ...
    # Rename: drop suffix, fixed filename
    summary_file = output_dir / "entity_comparison_gpt_lib.json"
...
def process_single_call(call_dir: Path, output_suffix: str = OUTPUT_SUFFIX_DEFAULT) -> Dict:
    ...
    # Rename: entities file -> entities_lib.json
    llm_entities_file = output_dir / "entities_lib.json"
    ...
    # Removed writing per-call detailed results file
...
def main():
    ...
    parser.add_argument("--output", default="eer_gpt_lib.json", 
                       help="Output JSON file for detailed results")
    # Rename run-level CSV default
    parser.add_argument("--csv-output", default="eer_summary_gpt_lib.csv",
    ...
    for cdir in call_dirs:
        try:
            # Updated per-call entities filename
            with open(cdir / "output" / "entities_lib.json", 'r', encoding='utf-8') as f:
                data = json.load(f)
    ...
    print(f"Individual call summaries saved in each call's output/entity_comparison_gpt_lib.json")

if __name__ == "__main__":
    # Mirror stdout/stderr to output_lib.txt in addition to console
    try:
        log_path = Path(__file__).parent / "output_lib.txt"
